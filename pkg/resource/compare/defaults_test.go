...
func TestCompareRoutes(t *testing.T) {
	...
	assert.True(t, equalsRoute(&routes[0], &routes[1]), "Expected resources to be deemed equal based on route comparator")
}
...
func TestCompareServices(t *testing.T) {
	...
	assert.True(t, equalsService(&services[0], &services[1]), "Expected resources to be deemed equal based on service comparator")
}
...
func TestCompareDeploymentConfigs(t *testing.T) {
	...
	assert.True(t, equalsDeploymentConfig(&dcs[0], &dcs[1]), "Expected resources to be deemed equal based on DC comparator")
}
